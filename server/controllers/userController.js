...
export const updateUserData = async (req, res) => {
    try {
        ...
        if(profile) {
            ...
            // Méthode correcte pour v7.x : imagekit.files.upload()
            const response = await imagekit.files.upload({
                file: buffer.toString('base64'),
                fileName: profile.originalname,
            })

            // Utilisation de helper.buildSrc() pour générer l'URL
...
export const sendConnectionRequest = async (req, res) => {
        try {
            ...
            if(connectionRequests.lenght >= 20) {
                return res.json({success: false, message: 'You have sent more than 20 connections request in the last 24 hours'})
            }
            ...
            if(!connection) {
                ...
                return res.json({success: false, message: 'You are already connected or have a pending request with this user'})
            }
